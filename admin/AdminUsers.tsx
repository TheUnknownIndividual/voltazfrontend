...
const AdminUsers: React.FC<{ adminSession: AdminUser; openActivityUserId?: number | null; onActivityOpened?: () => void }> = ({ adminSession, openActivityUserId, onActivityOpened }) => {
  ...
  const togglePage = (page: AdminPage) => setDraft((current) => ({ ...current, allowedPages: current.allowedPages.includes(page) ? current.allowedPages.filter((item) => item !== page) : [...current.allowedPages, page] }));
  const payload = { ...draft, monthlySalary: draft.monthlySalary.trim() ? Number(draft.monthlySalary) : (editing ? undefined : null), clearSalary: editing ? draft.clearSalary : undefined, telegramChatId: draft.telegramChatId.trim() ? Number(draft.telegramChatId) : null };
  const save = async () => {
    if (!draft.displayName.trim() || (!editing && (!draft.username.trim() || !draft.password.trim()))) { showNotification('İstifadəçi adı, görünən ad və şifrə daxil edilməlidir.', 'warning'); return; }
    try {
      if (editing) { await updateAdminAccess(editing.id, payload); if (draft.password.trim()) await resetAdminPassword(editing.id, draft.password.trim()); }
      else {
...
const Label = ({ title, wide, children }: { title: string; wide?: boolean; children: React.ReactNode }) => <label className={`text-xs font-bold text-slate-700 ${wide ? 'md:col-span-2' : ''}`}>{title}{children}</label>;
