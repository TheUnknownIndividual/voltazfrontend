  ...
  return (
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editingId) {
                await updatePartnershipType(editingId, payload);
                showNotification("Application type updated", "success");
            } else {
                await createPartnershipType(payload);
...
const handleEdit = async (id: any) => {
    try {
        ...
        const res = await getPartnershipTypeById(id);
    ...
    const handleDelete = async (id: string) => {
        try {
            await deletePartnershipType(id);
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-6">
                <div>
                    <h2 className="text-2xl font-black text-slate-900 uppercase tracking-widest">Tərəfdaşlıq tənzimləmləmləri</h2>
                    <p className="text-slate-500 text-sm">Tərəfdaşlıq müraciətlərini 4 dildə idarə edin.</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl overflow-hidden">
                ...
                <div className="p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-8">
                        {partnershipTypes.map((item: any) => (
                            <div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group gap-3">
