...
import { buildInitialAssessmentDocx, type SolarSystemType } from '../utils/docxTemplate';
...
interface GrowattInverterOption {
  model: string;
...
interface InverterRecommendation extends GrowattInverterOption {
  quantity: number;
...
const PANEL_MANUFACTURER_WARRANTY_YEARS = 25;
const INVERTER_MANUFACTURER_WARRANTY_YEARS = 5;
...
const growattInverters: GrowattInverterOption[] = ([
  {
    model: 'Growatt MIN 5000TL-X On-Grid inverter',
    phase: 'single',
    nominalAcKw: 5,
    maxDcKw: 7.5,
    mppt: 2,
    ...
    mpptRange: '80-550 V',
    maxDcVoltage: 550,
    maxInputCurrent: '13.5 A / MPPT'
  },
  {
    model: 'Growatt MIN 6000TL-X On-Grid inverter',
    phase: 'single',
    nominalAcKw: 6,
    maxDcKw: 9,
    mppt: 2,
  ...
  {
    model: 'Growatt MOD 10KTL3-X On-Grid inverter',
    phase: 'three',
    nominalAcKw: 10,
    maxDcKw: 15,
    mppt: 2,
    inputs: 2,
    mpptRange: '140-1000 V',
    maxDcVoltage: 1100,
    maxInputCurrent: '16 A / MPPT'
  },
  {
    model: 'Growatt MID 15KTL3-X On-Grid inverter',
    phase: 'three',
    nominalAcKw: 15,
    maxDcKw: 22.5,
    mppt: 2,
    ...
    mpptRange: '140-1000 V',
    maxDcVoltage: 1100,
    maxInputCurrent: '26 A / MPPT'
  },
  {
    model: 'Growatt MID 20KTL3-X On-Grid inverter',
    phase: 'three',
    nominalAcKw: 20,
    maxDcKw: 30,
    mppt: 2,
    ...
    mpptRange: '140-1000 V',
    maxDcVoltage: 1100,
    maxInputCurrent: '32 A / MPPT'
  },
  {
    model: 'Growatt MID 30KTL3-X On-Grid inverter',
    phase: 'three',
    nominalAcKw: 30,
    maxDcKw: 45,
    mppt: 3,
    inputs: 6,
    mpptRange: '200-1000 V',
    maxDcVoltage: 1100,
    maxInputCurrent: '32 A / MPPT'
  },
  {
    model: 'Growatt MAX 50KTL3 LV On-Grid inverter',
    phase: 'three',
    nominalAcKw: 50,
    maxDcKw: 75,
    mppt: 6,
  ...
  {
    model: 'Growatt MAX 100KTL3-X LV On-Grid inverter',
    phase: 'three',
    nominalAcKw: 100,
    maxDcKw: 150,
    mppt: 10,
...
] satisfies Omit<GrowattInverterOption, 'warrantyYears'>[]).map((inverter) => ({
  ...inverter,
  warrantyYears: INVERTER_MANUFACTURER_WARRANTY_YEARS
}));
...
const ui: Record<Lang, Record<string, string>> = {
  az: {
    ...
    recommendedInverter: 'Tövsiyə olunan Growatt inverter',
  ...
  en: {
    ...
    recommendedInverter: 'Recommended Growatt inverter',
  ...
  ru: {
    ...
    recommendedInverter: 'Рекомендуемый инвертор Growatt',
  ...
  tr: {
    ...
    recommendedInverter: 'Önerilen Growatt inverter',
...
const getInverterCompatibilityNote = (panelCount: number, panelWattage: number, inverter: GrowattInverterOption) => {
  ...
  return `${inverter.mppt} MPPT, ${inverter.inputs} giriş, MPPT diapazonu ${inverter.mpptRange}, max. DC ${inverter.maxDcVoltage} V, max. giriş cərəyanı ${inverter.maxInputCurrent}; ${panelCount} panel üçün ${estimatedPanelCurrent}. String sayı və gərginlik mühəndis yoxlamasında təsdiqlənməlidir.`;
};

const getInverterSpec = (recommendation: InverterRecommendation) =>
  `${formatNumber(recommendation.nominalAcKw, 1)} kW AC, ${recommendation.phase === 'three' ? '3 faza' : '1 faza'}; max. DC ${formatNumber(recommendation.maxDcKw, 1)} kWp; ${recommendation.mppt} MPPT; ${recommendation.inputs} giriş; MPPT ${recommendation.mpptRange}; max. DC ${recommendation.maxDcVoltage} V; max. giriş cərəyanı ${recommendation.maxInputCurrent}.`;

const recommendGrowattInverter = (systemKw: number, panelCount: number, panelWattage: number, phase: ConnectionPhase): InverterRecommendation => {
  const phaseOptions = growattInverters
    .filter((inverter) => inverter.phase === phase)
    .sort((a, b) => a.maxDcKw - b.maxDcKw);
  const exactOption = phaseOptions.find((inverter) => inverter.maxDcKw >= systemKw);
  const selected = exactOption || phaseOptions[phaseOptions.length - 1] || growattInverters[growattInverters.length - 1];
  const quantity = exactOption ? 1 : Math.max(1, Math.ceil(systemKw / selected.maxDcKw));
  const recommendation: InverterRecommendation = {
    ...selected,
    quantity,
    totalNominalAcKw: selected.nominalAcKw * quantity,
    totalMaxDcKw: selected.maxDcKw * quantity,
    isPreliminary: true,
    compatibilityNote: getInverterCompatibilityNote(panelCount, panelWattage, selected)
  };
...
const buildProposalPdf = (result: CalculationResult, formulas: string[]) => {
  const commands: string[] = [
    ...
    pdfText(320, 632, 10, 'Estimated price', false, '0.18 0.48 0.34 rg'),
    pdfText(320, 614, 22, `${formatNumber(result.estimatedCost)} manat`, true),
...
const AdminSolarCalculator: React.FC<{ lang?: Lang }> = ({ lang = 'az' }) => {
  ...
  const [address, setAddress] = useState<string>(draft.address || 'Bakı, Səbayıl');
  ...
  }, [address, annualSavings, cityName, connectionPhase, consumptionPeriod, consumptionValue, customBoqItems, customerType, documentCode, installationDays, inverterCount, inverterModel, inverterSpec, method, monthlyBill, mountType, panelModel, panelSpec, panelWattage, projectName, proposalStatus, systemKwInput, systemType, targetOffset, tariff]);
  ...
  const result = useMemo<CalculationResult>(() => {
    ...
    const inverter = recommendGrowattInverter(systemKw, panelCount, panelWattageValue, connectionPhase);
    ...
    return {
      ...
      paybackYears: calculatedAnnualSavings > 0 ? estimatedCost / calculatedAnnualSavings : 0,
      inverter,
      connectionPhase
    };
  }, [cityName, connectionPhase, consumptionPeriod, consumptionValue, customBoqItems, customerType, method, monthlyBill, mountType, panelWattage, systemKwInput, targetOffset, tariff, methods, t.groundMount, t.roofMount]);
  ...
  const buildAnalyticsPayload = (exportType: 'pdf' | 'docx', issuedDocumentNumber?: string) => ({
    ...
    result: {
      ...
      inverterQuantity: result.inverter.quantity
    },
    formulas
  });

  const validateExportProject = () => {
    if (!projectName.trim()) {
  ...
  const downloadPdf = async () => {
    ...
    try {
      await logAdminPdfExport(projectName.trim(), lang, buildAnalyticsPayload('pdf'));
  ...
  const downloadInitialAssessment = async () => {
    ...
    try {
      const issue = await issueAdminDocxExport(projectName.trim(), documentCode, lang, buildAnalyticsPayload('docx'));
      const annualSavingsValue =
        toPositiveNumber(annualSavings) || result.yearlyProduction * result.tariff;
      const basePrice = result.estimatedCost;
      const vat = basePrice * 0.18;
      const defaultInverterSpec = `${getInverterSpec(result.inverter)} ${result.inverter.compatibilityNote}`;
      ...
      const blob = await buildInitialAssessmentDocx({
        ...
        address: address.trim() || `${result.city.city}, ${result.city.region}`,
        ...
        annualSavingsAzn: annualSavingsValue,
        customerType: result.customerType,
        mountType,
        panelModel: panelModel.trim() || DEFAULT_PANEL_MODEL,
        panelSpec: panelSpec.trim() || defaultPanelSpec(result.panelWattage),
        inverterModel: inverterModel.trim() || result.inverter.model,
        inverterSpec: inverterSpec.trim() || defaultInverterSpec,
        inverterCount: Math.max(1, Math.round(toPositiveNumber(inverterCount) || result.inverter.quantity)),
        basePriceAzn: basePrice,
        vatAzn: vat,
        totalPriceAzn: basePrice + vat,
        installationDays: Math.max(1, Math.round(toPositiveNumber(installationDays) || 7)),
        customBoqItems: customDocItems
      });

      downloadBlob(blob, `${issue.documentNumber}-solarix-ilkin-qiymetlendirme.docx`);
      clearSolarCalculatorDraft();
    } catch (error) {
      console.error(error);
  ...
  const ResultCard = ({ label, value, note }: { label: string; value: string; note?: string }) => (
    <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm">
      ...
      {note && <div className="mt-2 break-words text-xs font-medium leading-relaxed text-slate-500">{note}</div>}
    </div>
  );
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        <div className="xl:col-span-5 bg-white rounded-[2rem] border border-slate-100 p-6 md:p-8 shadow-xl space-y-6">
          <div className="rounded-3xl border border-emerald-100 bg-emerald-50 p-5">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="relative md:col-span-2">
                ...
                <p className="mt-2 text-xs font-semibold text-emerald-800/70">Tapılmasa, ixrac zamanı avtomatik yeni satış layihəsi yaradılacaq.</p>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:border-emerald-500"
              >
                <option value="roof">{t.roofMount} - 850 AZN/kW</option>
                <option value="ground">{t.groundMount} - 1100 AZN/kW</option>
              </select>
            </div>
          ...
          <div className="rounded-3xl border border-slate-100 bg-slate-50 p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.inverterCount}</label>
                <input
                  type="number"
                  min="1"
                  value={inverterCount}
                  placeholder={String(result.inverter.quantity)}
            ...
            <details className="group">
              ...
              <div className="mt-4 space-y-4">
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.inverterModel}</label>
                  <input
                    value={inverterModel}
                    placeholder={result.inverter.model}
                    onChange={(event) => setInverterModel(event.target.value)}
                    className="w-full rounded-2xl border border-slate-100 bg-white px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:border-emerald-500"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t.inverterSpec}</label>
                  <textarea
                    value={inverterSpec}
                    placeholder={`${getInverterSpec(result.inverter)} ${result.inverter.compatibilityNote}`}
        ...
        <div className="xl:col-span-7 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            />
            <ResultCard
              label={t.actualDcPower}
              value={`${formatNumber(result.systemKw, 2)} kW`}
              note={`${result.panelCount} x ${formatNumber(result.panelWattage)}W ${t.panelCount.toLowerCase()}`}
            />
            <ResultCard
              label={t.estimatedPrice}
              value={formatMoney(result.estimatedCost)}
              note={
                result.boqAdditionsCost > 0
                  ? `${t.fullSystemPrice}: ${formatMoney(result.baseSystemCost)} + ${t.boqAdditionsTotal}: ${formatMoney(result.boqAdditionsCost)}`
                  : `${t.fullSystemPrice}: ${formatNumber(result.pricePerKw)} AZN/kW - ${result.mountLabel}`
              }
            />
            ...
            <ResultCard label={t.annualSavings} value={formatMoney(result.annualSavings)} note={`${formatNumber(result.tariff, 3)} AZN/kWh`} />
            <ResultCard label={t.payback} value={`${formatNumber(result.paybackYears, 1)} ${t.years}`} note={t.connectionWarning} />
            <ResultCard
              label={t.recommendedInverter}
              value={`${result.inverter.quantity} x ${result.inverter.model}`}
              note={`${formatNumber(result.inverter.totalNominalAcKw, 1)} kW AC / ${formatNumber(result.inverter.totalMaxDcKw, 1)} kWp DC max`}
            />
            <ResultCard label={t.panelWarranty} value={`${PANEL_MANUFACTURER_WARRANTY_YEARS} ${t.years}`} note={t.manufacturerWarrantyNote} />
            <ResultCard label={t.inverterWarranty} value={`${INVERTER_MANUFACTURER_WARRANTY_YEARS} ${t.years}`} note={t.preliminaryMark} />
            <ResultCard label={t.requiredArea} value={`${formatNumber(result.roofArea)} m²`} note={`${formatNumber(PANEL_AREA, 1)} ${t.areaNote}`} />
            <ResultCard label={t.proposalStatus} value={t[proposalStatus]} note={t.azExclusion} />
          </div>
