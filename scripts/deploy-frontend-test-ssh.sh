...
REMOTE_TARGET="C:/inetpub/wwwroot/testvoltaz"
...
DEPLOY_TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/volt-frontend-ssh.XXXXXX")"
BUILD_DIR="$DEPLOY_TEMP_DIR/dist"
PACKAGE_NAME="testvoltaz-frontend-$(date +%Y%m%d-%H%M%S).zip"
...
echo "Building test frontend without prerender..."
VITE_API_BASE_URL="https://test.api.volt.az/api/" \
...
VITE_APPLE_REDIRECT_URI="${VITE_APPLE_REDIRECT_URI:-https://test.volt.az/api/auth/callback/apple}" \
...
if [ ! -f "$BUILD_DIR/index.html" ] || [ ! -f "$BUILD_DIR/web.config" ] || [ ! -d "$BUILD_DIR/assets" ]; then
  echo "Build output is incomplete; deployment stopped." >&2
  exit 1
fi
...
echo "Installing on the test site..."
...
# Windows PowerShell can return success for some -File path failures. The
# installer removes the package only after a completed deployment, so a
# retained package is a reliable failure signal.
ssh "${SSH_OPTIONS[@]}" "$SSH_USER@$SSH_HOST" \
...
echo "Checking test.volt.az..."
curl --fail --silent --show-error --location --max-time 30 \
  --output /dev/null \
  "https://test.volt.az/"
curl --fail --silent --show-error --location --max-time 30 \
  --output /dev/null \
  "https://test.volt.az/assets/$PDF_WORKER_FILE"

echo "SSH test deployment finished successfully."
