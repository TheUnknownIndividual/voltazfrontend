...
const InfoSection: React.FC<InfoSectionProps> = ({ lang = 'az', onNavigate }) => {
  ...
  return (
    <section id="legislation" className="relative overflow-hidden bg-[var(--color-dark)] py-8 md:py-20">
      ...
      <div className="relative mx-auto max-w-[1440px] px-4 md:px-12">
        <div className="grid overflow-hidden rounded-[2rem] border border-white/10 shadow-2xl shadow-black/15 lg:grid-cols-2">
          <div className="flex flex-col p-6 md:p-10 lg:min-h-[720px] lg:p-12" style={{ backgroundColor: 'color-mix(in srgb, var(--color-dark) 95%, transparent)' }}>
            <header className="mb-5 md:mb-8">
              ...
              <h2 className="mb-4 max-w-2xl text-3xl font-black leading-tight tracking-tight text-white md:text-4xl">{t.title}</h2>
              <p className="max-w-xl text-sm leading-7 text-slate-300">{t.intro}</p>
            </header>
            ...
            <div className="border-t border-white/10">
              {t.steps.map((step, index) => {
                ...
                return (
                  <article key={step[0]} className="flex gap-4 border-b border-white/10 py-4 md:gap-5 md:py-5">
                    <div className="flex h-11 w-11 flex-none items-center justify-center rounded-xl bg-white/10 text-[var(--color-primary)]">
                    ...
                    <div>
                      ...
                      <p className="text-xs leading-5 text-slate-400 md:text-sm">{step[2]}</p>
                    </div>
                  </article>
                );
              })}
            ...
            <div className="flex gap-4 pt-4 md:pt-5">
              <div className="flex h-11 w-11 flex-none items-center justify-center rounded-xl bg-white/10 text-[var(--color-primary)]">
                <CloudSun className="h-5 w-5" strokeWidth={1.8} aria-hidden="true" />
              </div>
              <div>
          ...
          <div className="flex min-h-0 items-stretch justify-center p-0 md:min-h-[620px] md:items-center md:p-10 lg:min-h-[720px]">
            <aside className="w-full max-w-none rounded-b-[2rem] rounded-t-none bg-[var(--color-primary)] p-6 text-[var(--color-dark)] shadow-2xl shadow-black/20 md:max-w-md md:rounded-[2rem] md:p-8">
              <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--color-dark)] text-[var(--color-primary)]">
                <CircleDollarSign className="h-5 w-5" strokeWidth={1.8} aria-hidden="true" />
              </div>
              <h3 className="mb-5 text-xl font-black leading-tight md:text-2xl">{t.benefitTitle}</h3>
              <ul className="grid gap-3">
                {t.benefits.map(benefit => (
                  <li key={benefit} className="flex gap-3 text-sm font-medium leading-5 text-[var(--color-dark)] opacity-90">
              ...
              <button onClick={() => onNavigate?.('calculator')} className="group mt-6 flex w-full items-center justify-between rounded-xl bg-[var(--color-dark)] px-5 py-4 text-left text-[10px] font-black uppercase tracking-[0.12em] text-white transition-all hover:bg-white hover:text-[var(--color-dark)]">
