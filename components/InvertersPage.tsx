...
const InvertersPage: React.FC<Props> = ({ lang, initialPage, onNavigate, onSelectProduct, onOrderNow, onAddToCart }) => {
  ...
  return <main className="bg-white text-slate-900">
    <section className="relative isolate overflow-hidden bg-slate-950 px-4 pb-9 pt-5 text-white md:py-24">
      ...
      <div className="relative mx-auto max-w-7xl"><div className="md:ml-auto md:max-w-3xl">
        ...
        <div className="mt-6 grid gap-2.5 sm:flex sm:flex-wrap md:mt-8 md:gap-3">
          <button onClick={() => onNavigate('calculator')} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[var(--primary)] px-5 py-3 text-sm font-bold text-white transition-colors hover:bg-[var(--primary-hover)] sm:w-auto"><Calculator className="h-4 w-4" />{copy.calculator}</button>
    ...
    <section className="px-4 py-10 md:py-20"><div className="mx-auto max-w-5xl rounded-3xl bg-[var(--mint)] p-6 text-[var(--header-bg)] md:rounded-[2.5rem] md:p-12"><BatteryCharging className="h-7 w-7 md:h-8 md:w-8" /><h2 className="mt-3 text-2xl font-black leading-tight md:mt-4 md:text-3xl">{copy.finalTitle}</h2><p className="mt-3 max-w-3xl text-sm font-semibold leading-6 md:mt-4 md:text-base md:leading-7">{copy.finalText}</p><div className="mt-5 grid gap-2.5 sm:flex sm:flex-wrap md:mt-7 md:gap-3"><button onClick={() => onNavigate('calculator')} className="w-full rounded-lg bg-[var(--header-bg)] px-5 py-3 text-sm font-bold text-white sm:w-auto">{copy.calculator}</button><button onClick={() => onNavigate('contact')} className="w-full rounded-lg border border-[var(--primary)] bg-white px-5 py-3 text-sm font-bold text-[var(--header-bg)] sm:w-auto">{copy.contact}</button></div></div></section>
  </main>;
};
