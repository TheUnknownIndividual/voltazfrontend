...
const InvertersPage: React.FC<Props> = ({ lang, initialPage, onNavigate, onSelectProduct, onOrderNow, onAddToCart }) => {
  ...
  return <main className="bg-white text-slate-900">
    <section className="relative isolate overflow-hidden bg-slate-950 px-4 py-16 text-white md:py-24">
      <img src="/inverters-hero.webp" alt="" aria-hidden="true" fetchPriority="high" decoding="async" className="pointer-events-none absolute inset-0 -z-20 h-full w-full select-none object-cover object-center" />
      <div className="absolute inset-0 -z-10 bg-slate-950/30" />
      <div className="absolute inset-0 -z-10 bg-gradient-to-l from-slate-950/95 via-slate-950/75 to-slate-950/20" />
      <div className="relative mx-auto max-w-7xl"><div className="md:ml-auto md:max-w-3xl">
        ...
        <h1 className="mt-4 text-4xl font-black leading-tight md:text-6xl">{copy.title}</h1>
        <p className="mt-6 text-base font-medium leading-8 text-slate-200 md:text-lg">{copy.lead}</p>
        <p className="mt-3 text-sm leading-7 text-slate-300">{copy.detail}</p>
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <button onClick={() => onNavigate('contact')} className="inline-flex items-center gap-2 rounded-xl border border-white/25 bg-slate-950/20 px-5 py-3 text-sm font-black text-white">{copy.contact}<ArrowRight className="h-4 w-4" /></button>
        </div>
      </div></div>
    </section>
    ...
    <section className="mx-auto max-w-7xl px-4 pt-14 md:pt-20">
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><h2 className="text-3xl font-black md:text-4xl">{copy.catalogue}</h2><p className="mt-3 max-w-3xl leading-7 text-slate-600">{copy.catalogueText}</p></div><button onClick={() => onNavigate('products')} className="inline-flex w-fit items-center gap-2 font-black text-[#658300]">{copy.products}<ArrowRight className="h-4 w-4" /></button></div>
    </section>
